...
const handleSeatSelection = (seat) => {
 ...
 const reanderSeats = (row, count = 9) => {
  return (
          ...
          return (
            <button
              ...
              className={`w-8 h-8 rounded border border-primary/60 cursor-pointer ${
                selectedSeats.includes(seatNumber) ? 'bg-primary text-white' : ''
              }`}
            >
              {seatNumber}
            </button>
  ...
  return show ? (
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50">
      ...
      <div className="relative flex-1 flex flex-col items-center max-md:mt-16">
        ...
        <img src={assets.screenImage} alt="Screen" className="w-full max-w-2xl mb-8" />
        <p className="text-gray-400 text-sm mb-6">SCREEN SIDE</p>

        <div className="flex flex-col items-center mt-10 text-xs text-gray-300">
            <div className="grid grid-cols-2 md:grid-cols-1 gap-8 md:gap-2 mb-6">
              {groupRows[0].map((row) => reanderSeats(row))}  
            </div>

            <div className="grid grid-cols-2 gap-11">
              {groupRows.slice(1).map((group,idx) => 
                <div key={idx} className="flex flex-col items-center">  
                  {group.map((row) => reanderSeats(row))}
                </div>
              )}
              </div>
        </div>
